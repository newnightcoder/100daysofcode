...
let timeseriesPerCountry = timeseries+'?onlyCountries=true'
...
fetch(timeseriesPerCountry)
.then(function (r){
  return r.json();
})
.then(function (d){
  console.log('timeseries per country', d);
  let timePerCountry = d.map(function(d){
    return d.timeseries;
  })
  console.log('timeseries mapped',timePerCountry);

//   let sumCountries = 0;
//   for (i = 0; i<timePerCountry.length; i++){
//     return sumCountries += timePerCountry[i]..dataconfirmed;
//   }
//   console.log('sum', sumCountries);
})
...
// Styles the map in night mode.
  ...
  fetch(statsAllCountries)
    ...
    .then(function(data){
      console.log('statsAllCountries',data)
...
confirmedButton.addEventListener('click',function(){
  ...
  fetch(statsBrief)
    ...
    .then(function(data){
      ...
      document.querySelector('.h2mid').innerHTML='Confirmed cases<br>per country<br><span class="scroll">&lpar;scroll down&rpar;<span>';
    })
...
deathsButton.addEventListener('click',function(){
    ...
    .then(function(data){
      ...
      document.querySelector('.h2mid').innerHTML='Confirmed deaths<br>per country<br><span class="scroll">&lpar;scroll down&rpar;<span>';
    })
  ...
  fetch(statsAllCountries)
    ...
    .then(function(data){
      ...
      let htmlDeaths = sort.map(function(data){
        ...
        return `<div><span class="deathNumbersPerCountry">${countriesDeath}</span>&nbsp;&nbsp;${data.countryregion}</div>`;
      }).join('');
...
recoveredButton.addEventListener('click',function(){
  ...
  recoveredButton.style.color='#00ff00';
    ...
    .then(function(data){
      ...
      document.querySelector('.h2mid').innerHTML='Recoveries<br>per country<br><span class="scroll">&lpar;scroll down&rpar;<span>';
    })
...
let spreadTitle = d3.select(".spread").append("h3").html("The fast spread of COronaVIrus throughout the world started shortly before spring 2020...");
let economyTitle = d3.select(".economy").append("h3").html("Chapter 2<br>became in the spring the biggest threat of global economic collapse since 1921's Black Thursday ...");
let socialTitle = d3.select(".social").append("h3").html("Chapter 3<br>... resulting in historic social consequences - some dramatic, some un-expected");
let spiritualTitle = d3.select(".spiritual").append("h3").html("Chapter 4<br>Leading many to open up to some form of spitituality or metaphysics");
let futureTitle = d3.select(".future").append("h3").html("EPILOGUE?<br>The future");
...
d3.json(timeseries)
  .then(function (data){
    ...
    let rawTimeseries = data;
    console.log('raw timeseries', rawTimeseries);

    let countryList = rawTimeseries.sort(function(a,b){
    ...
    const franceData = rawTimeseries[125].timeseries;
    ...
    console.log(recovDataset);

//----------------------- BUTTONS --------------------------------------------//
//------------------------------------------------------------------------------

  let confirmedChartButton = document.querySelector('.confirmedChartButton');
  let deathsChartButton = document.querySelector('.deathsChartButton');
  let recoveredChartButton = document.querySelector('recoveredChartButton');

  confirmedChartButton.addEventListener('onclick', function(){
...
//-----------------------------------------------------------------------------
  svgSpread.append('text')
           .text(function(){
             return rawTimeseries[125].countryregion;
           })
