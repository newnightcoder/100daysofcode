...
coords.forEach(
        // let r = radiusBasis.forEach(function (radius){
        //     return data.confirmed*10;
        // });
        // for (coords of data){
...
fetch(statsAllCountries)
  ...
  .then(function(data){
    let dateUpdate = new Date(data[0].lastupdate); //JS FORMATTING OF THE JSON ISO TIMESTAMP TO MAKE IT MORE READABLE!!! //2018-07-31T11:56:48Z - ISO string can be parsed using new Date("2018-07-31T11:56:48Z") and obtained from a Date object using dateObject.toISOString()
    let update = `<div>${dateUpdate}</div>`;
