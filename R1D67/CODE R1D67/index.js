...
function restartGame(){
  ...
  restartButton.innerHTML = 'click here to<br>RESTART GAME !';
...
function draw(){
  ...
  drawBricks();
  if (gameState!=='ending'){
    drawGameStats('SCORE: '+score, 450, canvas_height-1);
    if (life==0){
      drawGameStats('DEAD ! R.I.P.',(canvas_width/2)-250, canvas_height-1)
    } else if (life<=1){
      drawGameStats('WARNING: LAST LIFE !',(canvas_width/2)-250, canvas_height-1)
      } else if (life>=2){
        drawGameStats(life+' LIVES', (canvas_width/2)-250, canvas_height-1);
        }
    drawGameStats(levelName, 150, 30)
    if (gameState=='level1'||gameState=='playL1'){
