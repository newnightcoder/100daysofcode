...
let levelName=document.createElement('levelName');
...
function displayLevel(){
  ...
  if (gameState=='level3'){
    canvas.style.backgroundImage="url('img/tilelevel.png')";
...
function draw(){
  if (gameState!=='ending'){
    displayGameStats('SCORE: '+ score, 400, 40);
    if (life>3){
      displayGameStats(life+' lives', 40, 40);
    }
    if(life==3){
      displayGameStats('WARNING : 3 lives left', 40, 40);
    }
    if (life==2){
      displayGameStats(life+' lives', 40, 40);
    }
    if (life==1){
      displayGameStats('death is near', 40, 40);
    }
    if (life==0){
      displayGameStats('DEAD ! R.I.P.', 40, 40);
    }
  }
  drawPaddle();
  drawBall();
