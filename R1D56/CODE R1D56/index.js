...
function levelUp(){
  ...
  for (r=0; r<3; r++){
    for (c=0; c<brick.columns; c++){
      levelWon=!bricks[r][c].unbroken;//&&!bricks[1][c].unbroken&&!bricks[2][c].unbroken;
    }
    if (levelWon==true && gameState=='level1'){
        setGameState('level2');
        displayLevel();
      }
    if (levelWon && gameState=='level2'){
            setGameState('level3');
            displayLevel();
        }
...
function draw(){
  ...
  drawBricksL1();
    ...
    if (gameState=='level2'||gameState=='playL2'||gameState==!'pause'){
    drawBricksL2();
  }
